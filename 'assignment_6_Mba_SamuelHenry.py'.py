...
# PROGRAM: MEDICAL ENGINEERING & eHEALTH
# COURSE: BIOINFORMATICS
# ASSIGNMENT: Implementation cellular automata with "Conway's Game of Life rule"
...
def rndmGrd(m): # Randomized Grid for every cell
    return np.random.choice(vl, m * m, p=[.4, .6]).reshape(m, m)

def aglide(i, j, grd): # Glider for Cell(0,0)
    glide = np.array([[0, 0, 1],
                     [1, 0, 1],
                     [0, 1, 1]])
    grd[i:i+3, j:j+3] = glide

def aGlideG(i, j, grd): # Glider Gun on Cell(0,0)
